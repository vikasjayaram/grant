...
exports.authorize = async ({provider, input}) => {
  ...
  if (provider.require_pushed_authorization_requests && provider.token_endpoint_auth_method === 'client_secret_post') {
    params.client_secret = provider.secret;
  }
  if (provider.require_pushed_authorization_requests && provider.token_endpoint_auth_method === 'private_key_jwt') {
    var jwt = ({kid, x5t, secret}) => ({
      ...
      payload: {
        iss: provider.key,
        sub: provider.key,
        aud: provider.issuer || provider.access_url,
    ...
    var assertion = (() => {
      ...
      var {public_key, private_key, kid} = provider
      return oidc.sign(jwt({
        kid: kid ? kid : (private_key.kty ? oidc.kid(private_key) : undefined),
  ...
  if (provider.require_pushed_authorization_requests) {
    var options = {
      method: 'POST',
      url: provider.pushed_authorization_request_endpoint,
      form: qs.stringify(params)
    }
    try {
      var {body:output} = await request({...options});
      var parQueryString = qs.stringify({
        client_id: params.client_id,
        request_uri: output.request_uri
      });
      return {provider, input, output: `${url}?${parQueryString}`}
    } catch (err) {
