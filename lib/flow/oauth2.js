  ...
  if (provider.token_endpoint_auth_method === 'private_key_jwt') {
    var jwt = ({kid, x5t, secret}) => ({
      ...
      payload: {
        iss: provider.key,
        sub: provider.key,
        aud: provider.custom_params.issuer || provider.access_url,
    ...
    var assertion = (() => {
      ...
      var {public_key, private_key, custom_params} = provider
      return oidc.sign(jwt({
        kid: custom_params.kid ? custom_params.kid : (private_key.kty ? oidc.kid(private_key) : undefined),
